...
for(k in c("all", "up", "down")){
  for (j in names(term2gene)){
    for (i in names(dge_split)){
      ...
      # Perform GO enrichment analysis using the `enricher` function from clusterProfiler
      ...
      # Annotate the resulting data with contrast, database, and direction
...
### Output
```{r}
# Function to plot and save dot plots for a specific contrast
plot_and_save_dotplot <- function(data_split, contrast, topn = 10, topn.pref = "qval", qcut = enrich_cut, nchar = 60, 
                                  direction = "direction", group = "Contrast", dot = "NES", qval = "qvalue", 
                                  term.id = "ID", term.name = "Description", output_path) {
  if (contrast %in% names(data_split)) {
    filtered_data <- data_split[[contrast]]
    if (nrow(filtered_data) > 0) {
      # Create the dot plot
      dot_plot <- aamisc::dotplotEnrich(dt = filtered_data, 
                                        ...
                                        term.id = term.id, 
                                        term.name = term.name) +
        # Enhance the theme for better export
        theme(
...
# Specify the contrast and output path
contrast_to_plot <- "Metformin_vs_AF_RA"
output_path <- "../output/Metformin_vs_AF_RA_dotplot.png"

# Generate and save the dot plot for "Metformin_vs_AF_RA"
plot_and_save_dotplot(data_split = go_gsea_res_split, contrast = contrast_to_plot, output_path = output_path)
```

...
# Without batch effect removed
# Plot MDS
mds <- plotMDS(logCounts_noNA, plot = FALSE)
...
#Un-corrected
# Dimension 1 separates LA from RA
# Dimension 2 ....
...
# Testing for association between metadata variables and PCs
```{r}
library(gridExtra)
# Perform PCA using the prcomp function
pca_result <- prcomp(t(logCounts_noNA), center = TRUE, scale. = TRUE)  # Transpose for PCA on samples

# Create a data frame with the first four principal components
pca_data <- data.frame(
  ...
  PC4 = pca_result$x[, 4],  # Fourth principal component
  Sample = rownames(pca_result$x)  # Sample names
)

# Merge the PCA results with your metadata
pca_meta <- merge(pca_data, meta, by.x = "Sample", by.y = "Sample ID")

# List of metadata variables to plot
metadata_vars <- c("Group", "Region", "Age", "Sex", "Batch", "Weight")

# Generate PCA plots for each metadata variable
pca_plots <- list()
for (var in metadata_vars) {
  pca_plots[[var]] <- ggplot(pca_meta, aes_string(x = "PC1", y = "PC2", color = var)) +
    geom_point(size = 3) +
...
# Example: Calculate correlations between PC1/PC2 and continuous metadata
cor_age_PC1 <- cor.test(pca_meta$PC1, pca_meta$Age, method = "pearson")
cor_age_PC2 <- cor.test(pca_meta$PC2, pca_meta$Age, method = "pearson")

cor_weight_PC1 <- cor.test(pca_meta$PC1, pca_meta$Weight, method = "pearson")
cor_weight_PC2 <- cor.test(pca_meta$PC2, pca_meta$Weight, method = "pearson")

# Print correlation results
cat("Correlation between PC1 and Age:\n")
print(cor_age_PC1)
cat("\nCorrelation between PC2 and Age:\n")
print(cor_age_PC2)

cat("\nCorrelation between PC1 and Weight:\n")
print(cor_weight_PC1)
cat("\nCorrelation between PC2 and Weight:\n")
print(cor_weight_PC2)

# Example: Linear model to test association between PC1 and a categorical variable
lm_region_PC1 <- lm(PC1 ~ Region, data = pca_meta)
lm_region_PC2 <- lm(PC2 ~ Region, data = pca_meta)

summary(lm_region_PC1)  # Check the p-value and model summary for PC1
summary(lm_region_PC2)  # Check the p-value and model summary for PC2

# Repeat for other categorical variables
lm_sex_PC1 <- lm(PC1 ~ Sex, data = pca_meta)
lm_sex_PC2 <- lm(PC2 ~ Sex, data = pca_meta)

summary(lm_sex_PC1)  # Check for PC1
summary(lm_sex_PC2)  # Check for PC2

# Remove the batch effect using the Horse variable
logCounts_batchRemoved <- removeBatchEffect(
  logCounts_noNA, 
  batch = meta$Horse  # Specify Horse as the batch variable
)

# Perform PCA using the prcomp function on batch-corrected data
pca_result <- prcomp(t(logCounts_batchRemoved), center = TRUE, scale. = TRUE)  # Transpose for PCA on samples
...
# The full model matrix including both adjustment variables and the variable of interest (Condition, Sex, Batch)
# mod <- model.matrix(~ 0 + Condition + Sex + Batch, data = d$samples)
mod <- model.matrix(~ 0 + Condition, data = d$samples)
mod0 <- model.matrix(~ 1, data = d$samples)  # Null model with only the intercept

# Estimate the number of significant surrogate variables using the "be" method
n.sv <- num.sv(imputed_data_qrilc, mod, method = "be")
print(n.sv)  # Check the number of surrogate variables

# Estimate surrogate variables
svobj <- sva(imputed_data_qrilc, mod, mod0, n.sv = n.sv)

# Adjust for surrogate variables
modSv <- cbind(mod, svobj$sv)  # Combine the model matrix with the surrogate variables
mod0Sv <- cbind(mod0, svobj$sv)  # Combine the null model with the surrogate variables

colnames(modSv) <- make.names(colnames(modSv))

# Calculate p-values and adjust them using Benjamini-Hochberg (BH) method
pValues <- f.pvalue(imputed_data_qrilc, mod, mod0)
qValues <- p.adjust(pValues, method = "BH")

# Fit the model using limma with the adjusted model matrix (modSv)
fit <- lmFit(imputed_data_qrilc, modSv)
...
# Run limma whilst blocking for "horse"/Estimate Correlation due to Repeated Measures (Blocking for "Horse")
# Use the model matrix adjusted for surrogate variables
corfit <- duplicateCorrelation(imputed_data_qrilc, modSv, block = as.factor(d$samples$Horse))

# Fit the linear model using the design matrix with surrogate variables and block for repeated measures
fit <- lmFit(imputed_data_qrilc, modSv, block = as.factor(d$samples$Horse), correlation = corfit$consensus.correlation)
fit <- contrasts.fit(fit, contrasts = con)  # Apply the contrast matrix
fit <- eBayes(fit, robust = TRUE, trend = TRUE)  # Apply Empirical Bayes moderation
...
# Optionally, save the results to files
 openxlsx::write.xlsx(x = res_split, file = "../../../../Terminal/analysis/01_dge/output/dge_results_sva.xlsx", asTable = TRUE)
...
combat_data <- ComBat(dat = imputed_data_qrilc, batch = batch, mod = model.matrix(~ condition))
...
# Visualazing effects 
```{r}
logCounts_noNA <- na.omit(logCounts)  # Remove rows with NA values

# Create a DGEList object and define the design matrix
annot_reordered <- annot[match(rownames(logCounts_noNA), annot$Genes), ]
d <- DGEList(counts = logCounts_noNA, genes = annot_reordered, samples = meta)
design <- model.matrix(~0 + Condition, data = d$samples)

# Plot MDS for raw data (before any correction)
mds_raw <- plotMDS(logCounts_noNA, plot = FALSE)
mds_plot_raw <- aamisc::ggMDS(mds = mds_raw, meta = d$samples, color.by = "Group", shape.by = "Region") +
  ggtitle("MDS: Raw Data")

# Remove batch effect using paired design (Horse as blocking factor)
...
mds_plot_blocked <- aamisc::ggMDS(mds = mds_blocked, meta = d$samples, color.by = "Group", shape.by = "Region") +
  ggtitle("MDS: Blocked for Horse")
...
mds_plot_combat <- aamisc::ggMDS(mds = mds_combat, meta = d$samples, color.by = "Group", shape.by = "Region") +
  ggtitle("MDS: ComBat Adjusted")
...
# Generate the MDS plot with SVA-adjusted data
mds_sva <- plotMDS(logCounts_sva, plot = FALSE)
mds_plot_sva <- aamisc::ggMDS(mds = mds_sva, meta = d$samples, color.by = "Group", shape.by = "Region") +
  ggtitle("MDS: SVA Adjusted")
...
#Verdic - Combat adjustment did not add anything

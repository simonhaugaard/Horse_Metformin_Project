...
Protein <- fread("C:/Users/nks882/OneDrive - University of Copenhagen/Documents/GitHub/Horse_Metformin_Project/Proteomics/Terminal/analysis/01_dge/output/dge_results_sva.tsv.gz")
```

#Seperate into contrasts
```{r}
# Separate into contrasts and select the appropriate columns
# RNA filtering based on adj. P values 
metformin_vs_placebo_RNA_sig_up_LA <- RNA[Contrast == "met_vs_placebo_LA" & logFC > 0 & adj.P.Val < 0.05, .(GENENAME)]
metformin_vs_placebo_RNA_sig_up_RA <- RNA[Contrast == "met_vs_placebo_RA" & logFC > 0 & adj.P.Val < 0.05, .(GENENAME)]
metformin_vs_placebo_RNA_sig_down_LA <- RNA[Contrast == "met_vs_placebo_LA" & logFC < 0 & adj.P.Val < 0.05, .(GENENAME)]
metformin_vs_placebo_RNA_sig_down_RA <- RNA[Contrast == "met_vs_placebo_RA" & logFC < 0 & adj.P.Val < 0.05, .(GENENAME)]

# Protein filtering based on adj. P. values
metformin_vs_placebo_protein_sig_up_LA <- Protein[Contrast == "Metformin_vs_AF_LA" & logFC > 0 & adj.P.Val < 0.05, .(GeneName)]
metformin_vs_placebo_protein_sig_up_RA <- Protein[Contrast == "Metformin_vs_AF_RA" & logFC > 0 & adj.P.Val < 0.05, .(GeneName)]
metformin_vs_placebo_protein_sig_down_LA <- Protein[Contrast == "Metformin_vs_AF_LA" & logFC < 0 & adj.P.Val < 0.05, .(GeneName)]
metformin_vs_placebo_protein_sig_down_RA <- Protein[Contrast == "Metformin_vs_AF_RA" & logFC < 0 & adj.P.Val < 0.05, .(GeneName)]

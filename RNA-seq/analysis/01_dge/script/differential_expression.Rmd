...
**#TODO:** You don't need `meta` if you are using `meta_reordered` in your `generate_pheatmap_for_goid()`. Or the other way around... (It seems like this is ChatGPT generated, is it right? Nothing wrong with using ChatGPT though :) )
...
#TODO: Don't use count, use logCPM to view expression
...
#Answer from ALI: 
#1) You should NEVER use raw expression (count) values, always use logCPM or batch corrected logCPM
#2) I get the same heatmap values with the code. Note that ENSECAG00000000813 = SCN5A
ra_id <- grepl(pattern = "_RA$", x = colnames(logCPM_batchRemoved))
pheatmap(logCPM_batchRemoved[c("ENSECAG00000000813","ENSECAG00000023015"),ra_id], 
         cluster_rows = TRUE, 
         cluster_cols = TRUE, 
         scale = "row", 
...
#TODO: Include heatmap on ionchannels! 
